...
export function categorizeEvent(
    ...
    const blockedKeywords = ['xxx', 'izin', 'kongre', 'toplanti', 'off', 'yokum', 'cumartesi', 'pazar', 'disindayim'];
    ...
    const lowerNorm = normalizedTitle.toLowerCase().replace(/\u0307/g, '');
...
export function normalizeName(name: string): string {
    ...
    n = n.replace(/\u0307/g, '') // Remove combining dot (fixes İ -> i̇ issue)
        .replace(/i̇/g, 'i')
        .replace(/ı/g, 'i')
        .replace(/ş/g, 's')
        .replace(/ç/g, 'c')
